...
public class RenderFakeBlock extends RenderBlocks {
    ...
    public void setLightAndColor(double u2, double v2, int side) {
        if (this.enableAO) {
            ...
            tessellator.setBrightness(this.mixAoBrightness(this.brightnessTopLeft, this.brightnessTopRight, this.brightnessBottomLeft, this.brightnessBottomRight, u * v, v
                    * (1.0D - u), (1.0D - v) * u, (1.0D - u) * (1.0D - v)));
    ...
    public void renderSide(Block block, double x, double y, double z, double ox, double oy, double oz, int ax, int ay, int az, int bx, int by, int bz, IconConnected icon, int side) {
        ...
        for (int j = 0; j < 4; j++) {
            int i = getType(block, side, (int) x, (int) y, (int) z, ax * (int) u[j], ay * (int) u[j], az * (int) u[j], bx * (int) v[j], by * (int) v[j], bz * (int) v[j],
                    (int) (ox * 2.0D - 1.0D), (int) (oy * 2.0D - 1.0D), (int) (oz * 2.0D - 1.0D));
            icon.setType(i);
            ...
            if (b) {
                if (matchBlock(side, x + ax + bx, y + ay + by, z + az + bz)) {
                    if ((matchBlock(Facing.oppositeSide[sidea], x + ax + bx + cx, y + ay + by + cy, z + az + bz + cz))
                            ...
                            || (matchBlock(sidea, x + bx + cx, y + by + cy, z + bz + cz)) || (matchBlock(sideb, x + ax + cx, y + ay + cy, z + az + cz)))
                        return 4;
