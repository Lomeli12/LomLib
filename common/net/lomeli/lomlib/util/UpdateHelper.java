...
public class UpdateHelper {
    ...
    public void check(String modname, String updateUrl, int major, int minor,
            int revision) {
        checkForUpdates(modname, updateUrl, major, minor, revision);
    }

    private void checkForUpdates(String modname, String updateURL, int major,
            int minor, int revision) {
        int[] latestVersion = {
                XMLUtil.getInteger(updateURL, "majorBuildNumber"),
                XMLUtil.getInteger(updateURL, "minorBuildNumber"),
        ...
        for(int i = 0; i < 3; i++) {
            if(latestVersion[i] > currentVersion[i]) {
                ...
                System.out.println("A new version of " + modname
                        + " can be downloaded at " + downloadURL);
            }
        }
    }
}
